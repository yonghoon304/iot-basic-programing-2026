...
- 윈도우 탐색기 > `보기` > `표시` > `파일 확장명` 활성화, 각파일 확장자 확인 필요

1. 폴더 및 파일 경로
    - 절대경로 - `C:\DEV\IDE\Microsoft VS Code\591199df40\resources\app\LICENSE.rtf` 와 같이 드라이브부터 전체 경로 모두 기재
        - 장점 - 정확한 경로가 표시
        - 단점 - 경로 문자열이 아주 길어질 수 있음
    - 상대경로 - `.\LICENS.rtf`와 같이, .(현재폴더), ..(부모폴더) 기호를 사용해서 짧게 경로를 표시
        - 장점 - 경로를 짧게 표시
        - 단점 - 개발자가 직접 경로가 어떤식으로 구성되는 파악 필요. 같은 폴더에 존재하는 경우 사용 가능

2. 파일 입출력 : [확인](./Day09/Chap18-Solution/Chap18-01-app/main.c)
    - 키보드 등 입력은 버퍼를 통한 입력 스트림(stram)에 출력은 출력 스트림(stram)에 보관
    - 파일 ,콘솔 등에 저장

    ![alt text](image-15.png)

    - fgetc() - 파일에서 글자 하나씩 읽는 함수. 파일의 마지막(EOF -End of File)까지 하나씩 읽는 함수
    - fputc() - 파일에 글자를 한자씩 출력(쓰는)하는 함수
    - fseek(),rewind() - 파일에 처음으로 돌아가서 다시 읽기
    - feof() - 파일의 끝까지 다 읽었는지 확인 함수
    - `fgets()` - 파이르이 커서(현재위치)에서 한줄을 읽는 함수
    - `fputs()` - 파일에 한줄을 출력(쓰는) 함수
    - fscanf() - 포맷에 맞춰서 파일에서 읽기 함수
    - fprintf() - 포맷에 맞춰서 파일에 쓰기 함수
    - stdio.h - stdin(표준입력),stdout(표준출력),stderr(표준에러) 포함
    - 인코딩 - EUC-KR(CP-949,멀티바이트,2byte로 한글 표현), UTF-8(3byte 한글) 

3. 파일 입출력 모드 - 바이너리,텍스트 : [확인](./Day09/Chap18-Solution/Chap18-05-app/main.c)
    - r - 읽기(read) 모드
    - w - 쓰기(write) 모드
    - a - 추가(append) 모드
    - t - 텍스트 모드(생략)
...
4. 한글 깨짐 현상 : [확인](./Day09/Chap18-Solution/Chap18-02-app/main.c)
...
6. 전처리 지시자 : [확인](./Day09/Chap19-Solution/Chap19-01-app/main.c)
...
7. 분할 컴파일 : [확인](./Day09/Chap19-Solution/Chap19-04-app/main.c)
    - 여러 파일을 분할해서 여럿이 같이 개발 ,프로그램 크기 줄이기, 코드 재활용 등
    - 전역변수 static - 외부에 공개하지 않는 내부용 정적 변수
    - ~~전역변수에 static은 사용하지 말 것~~
    - 외부 소스에서는 `extern`으로 공유 : [확인](./Day09/Chap19-Solution/Chap19-03-app/main.c)
...
1. **static(19장 static 오류 수정)**
    - 정적 키워드는 소스코드가 동작하는 동안 끝까지 살아남은 변수,함수
    - static으로 선언하면 자기 소스내에서는 위와 같이 동작
    - 외부 소스로 공유하지 않음 <-> `객체지향 언어와 정반대!`
    - 외부 소스에 공유하지 않을 변수를 static으로 지정할 것
    - 두 소스에 같은 이름의 함수나 변수를 쓰는데 중복문제를 해결하기 위해
